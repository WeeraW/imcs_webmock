class Staffs::Sales::FullfilledOrdersController < ApplicationController
  ...
  def search_orders
    @orders = Order::Order.includes(order_details: [:product], shipping_address: [:freight_provider])
  ...
  def find_fullfilled_orders
    @orders = Order::Order.includes(order_details: [:product], shipping_address: [:freight_provider]).fullfilled_orders_for_sale(current_staff.id).page(params[:page]).per(params[:per_page])
  end
end
