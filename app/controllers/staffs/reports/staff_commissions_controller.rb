class Staffs::Reports::StaffCommissionsController < ApplicationController
  ...
  def find_paid_orders_in_date_range!
    @begin_date = params[:beginning_date].blank? ? Date.today.beginning_of_month : Date.parse(params[:beginning_date])
    @end_date = params[:end_date].blank? ? Date.today.end_of_month : Date.parse(params[:end_date])
    @staff_orders = Staff.joins(:created_orders).group(:id).where(['order_orders.paid_approve_by_staff_id IS NOT ?', nil]).where(created_at: @begin_date..@end_date)
